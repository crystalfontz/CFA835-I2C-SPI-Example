...
#define STANDALONE_LOOP_DELAY 100 /*=4 updates/sec*/

//debugging information
#define STANDALONE_EXTENDED_INFO /*show extra packet count info on display*/
#define DEBUG_EXTRA_PACKET_INFO  /*print packet in/out info to the host pc*/
...
void standaloneMode(void) {
  ...
  while (1) {
    ...
    int32_t diff = tempSent - tempRecv;
    outPacket.length += sprintf((char *)&outPacket.data[outPacket.length], "Diff   : %08ld ", diff);
    sendPacket(&outPacket);
    countSent++;
    recieveData(IFACE_READY_WAIT);
#endif
...
bool getPacketFromFIFO(FIFO_t *FIFO, CFAPacket_t *packet) {
  ...
  for (positionCount = 0; positionCount < inQueue - (PACKET_HEADER_SIZE - 1); positionCount++) {
    ...
    if (
      ...
      (packet->length < PACKET_CFA835_DATA_SIZE + 1) && (inQueue > (positionCount + packet->length + PACKET_HEADER_SIZE - 1))) {
...
const uint16_t CRCLookupTable[256] = { 0x00000, 0x01189, 0x02312, 0x0329B, 0x04624, 0x057AD, 0x06536, 0x074BF,
